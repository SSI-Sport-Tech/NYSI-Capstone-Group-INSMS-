...
**Version:** 3.0
**Status:** Production Schema - Phase 2.5 Complete (Vectorization, Webscraper, Staging)
...
│   ├── webscraper_catalog_url               # Catalog URLs for scraping
...
│   ├── Athlete_Registry                     # Carding status (1:1)
│   ├── Athlete_Medical                      # Medical info (1:1)
...
- Manual supplements: Node.js calls `shared/vectorization.js` → `/api/vectorization/generate` (per vector)
- Staging approval: Node.js calls `/api/vectorization/generate-product-vectors` (generates both vectors at once)
- Supplement updates: Re-vectorized only if ingredients or nutritional info changed
- Stored as PostgreSQL vector(384) type
- Used for cosine similarity search (alternatives) and duplicate detection (staging)
...
**Purpose:** Temporary storage for web scraper data before admin review and approval

**Status:** Active — used by webscraper pipeline

**When Used:**
- Web scraper deposits new supplement data here
- Admin reviews, edits, and approves entries
- Approval workflow generates vectors, checks for duplicates, then promotes to SSS.Supplement
...
|--------|------|-------------|
| `is_reviewed` | BOOLEAN | false = pending review, true = approved/processed |
| `promoted_to_supplement_id` | UUID | Links to the created Supplement after approval |
| `webscraper_catalog_url_id` | UUID | Source catalog URL that produced this entry |

**Approval Workflow:**
1. Admin selects staging entries to approve
2. For each entry: validate required fields (packaging_form_id, status_id)
3. Apply batch_testing_org business rules
4. **Generate vectors** via Python service (`POST /api/vectorization/generate-product-vectors`)
5. **Check for duplicates** — 95% vector similarity + normalized name/brand match
6. If duplicate found → auto-delete from staging, return "duplicate" status
7. If not duplicate → INSERT into SSS.Supplement with vectors, mark staging as reviewed

**Duplicate Detection (95% Threshold):**
```javascript
// Normalize for comparison: lowercase, remove spaces and special chars
function normalizeForComparison(str) {
    return str?.toLowerCase().replace(/[^a-z0-9]/g, '') || '';
}

// Check: vector similarity >= 0.95 AND normalized name+brand match
// Uses cosine similarity via pgvector <=> operator
```

---

### SSS.webscraper_catalog_url

**Purpose:** Store catalog URLs that the web scraper targets
...
|--------|------|-------------|
| `id` | UUID | Primary key |
| `product_catalog_website` | TEXT | Catalog URL to scrape |
| `is_active` | BOOLEAN | Active/inactive for scraping |
| `number_of_catalog_page` | INTEGER | Number of pages (set by scraper, NULL initially) |

**Business Rules:**
- Duplicate URLs are prevented (checked before insert)
- Only active URLs (`is_active = true`) are sent to the scraper
...
**Staging Approval (vector similarity + name + brand):**
```javascript
// 95% vector similarity threshold
const DUPLICATE_SIMILARITY_THRESHOLD = 0.95;

// Step 1: Find supplements with >= 95% vector similarity
// Step 2: Among those, check if normalized name+brand matches
// Step 3: If match found → duplicate (auto-delete from staging)
// Step 4: If no match → safe to promote to main library
```

### 3. Vectorization on Update

**Re-vectorize only when relevant fields change:**
```javascript
// In supplement update controller:
const vectorFields = ['supplement_ingredient', 'nutritional_info_per_100g', 'nutritional_info_per_serving'];
const needsRevectorization = vectorFields.some(f => validated[f] !== undefined);

if (needsRevectorization) {
    // Merge existing data with updates, then regenerate vectors
    const mergedIngredients = validated.supplement_ingredient || existing.supplement_ingredient;
    const mergedPer100g = validated.nutritional_info_per_100g || existing.nutritional_info_per_100g;
    const mergedPerServing = validated.nutritional_info_per_serving || existing.nutritional_info_per_serving;
    // Call Python service to regenerate both vectors
}
```
...
**Last Updated:** February 5, 2026
**Next Review:** After AMS Phase 2 (assignments, mappings)
