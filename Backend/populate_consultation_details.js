...
const consultationData = {
  main_nutrition_diagnosis:
    "Inadequate carbohydrate intake for training load, insufficient protein post-exercise recovery, and suboptimal hydration during training sessions as evidenced by 24-hour dietary recall showing 3.2g/kg carbohydrates, 1.1g/kg protein, and reported fatigue during afternoon training.",
  ...
  other_review:
    "Supplement Intake - Athlete currently taking whey protein post-workout and multivitamin daily",
  follow_up_note:
    "Monitor carbohydrate intake around training sessions. Recommend 1.5g/kg carbs 2-3 hours pre-training and 1g/kg within 30 minutes post-training. Schedule follow-up in 2 weeks to assess energy levels and training performance.",
  intervention_note:
    "Prescribed increased carbohydrate intake targeting 5-7g/kg body weight daily. Recommended high-quality protein sources post-workout (25-30g). Advised on improved hydration strategy with electrolyte replacement during sessions >60 minutes.",
  other_remarks:
    "Athlete is highly motivated and receptive to dietary changes. Has good understanding of sports nutrition principles. Previously responded well to structured meal planning.",
};
...
async function populateConsultationDetails() {
  ...
  try {
    ...
    console.log(
      `Session note exists: ${checkResult.rows.length > 0 ? "Yes" : "No"}`,
    );
    ...
    if (verifyResult.rows.length > 0) {
      ...
      console.log(
        "Main Diagnosis:",
        data.main_nutrition_diagnosis?.substring(0, 100) + "...",
      );
      ...
      console.log(
        "Follow-up Note:",
        data.follow_up_note?.substring(0, 80) + "...",
      );
      console.log(
        "Intervention Note:",
        data.intervention_note?.substring(0, 80) + "...",
      );
      console.log(
        "Other Remarks:",
        data.other_remarks?.substring(0, 80) + "...",
      );
    }
  } catch (error) {
