...
function extractBatchIdFromText(text) {
    ...
    const patterns = [
        ...
        { regex: /(?:BATCH|BATCH\s*NO|BATCH\s*#|BATCH\s*NUMBER)[\s:\.]*([A-Z0-9\-:]{5,20})/i, type: 'batch', confidence: 'high' },
        { regex: /(?:LOT|LOT\s*NO|LOT\s*#|LOT\s*NUMBER)[\s:\.]*([A-Z0-9\-:]{5,20})/i, type: 'lot', confidence: 'high' },
        { regex: /(?:B\/?N|L\/?N)[\s:\.]*([A-Z0-9\-:]{5,20})/i, type: 'batch', confidence: 'medium' },

        // Alphanumeric codes
        { regex: /\b(BN\d{5,10})\b/, type: 'batch', confidence: 'high' },
        { regex: /\b(LN\d{5,10})\b/, type: 'lot', confidence: 'high' },
        { regex: /\b(IS[\-]?\d{5,10})\b/, type: 'certification', confidence: 'high' },
        { regex: /\b(NSF[\-]?\d{5,10})\b/, type: 'certification', confidence: 'high' },
            ...
            for (const match of matches) {
                ...
                if (extracted && extracted[1]) {
                    ...
                    if (!isLikelyDateOrPhone(batchId)) {
                        foundIds.push({ batch_id: batchId, batch_id_type: type, confidence });
                    }
                }
            }
        }
    ...
    if (foundIds.length > 0) {
        ...
        foundIds.sort((a, b) => confidenceOrder[a.confidence] - confidenceOrder[b.confidence]);

        const best = foundIds[0];
        const alternatives = foundIds.slice(1, 5)
 ...
 */
export async function findSimilarSupplements(vectorPerServing, vectorPer100g, page = 1, pageSize = 10) {
    ...
    if (!hasVectorPerServing && !hasVectorPer100g) {
        return { supplements: [], total: 0, page, per_page: pageSize, total_pages: 0 };
    }

    // Convert vectors to PostgreSQL array format string
    const vectorServingStr = hasVectorPerServing ? `[${vectorPerServing.join(',')}]` : null;
    const vector100gStr = hasVectorPer100g ? `[${vectorPer100g.join(',')}]` : null;
    ...
    let similarity100gExpr, similarityServingExpr, maxSimilarityExpr, matchedVectorExpr;
    ...
    return {
        supplements: result.rows.map(row => ({
            ...
            similarity_score: row.max_similarity ? parseFloat(row.max_similarity).toFixed(4) : '0',
            matched_vector: row.matched_vector,
            similarity_details: {
                per_100g: row.similarity_100g ? parseFloat(row.similarity_100g).toFixed(4) : null,
                per_serving: row.similarity_perserving ? parseFloat(row.similarity_perserving).toFixed(4) : null
            }
        })),
        total,
        page,
