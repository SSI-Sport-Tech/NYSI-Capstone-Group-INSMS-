...
const CODE_EXPIRY_MINUTES =
  parseInt(process.env.VERIFICATION_CODE_EXPIRY) || 15;
const MAX_VERIFICATION_ATTEMPTS = parseInt(
...
export async function register(req, res) {
  try {
    ...
    if (nutritionistRoles.includes(newUser.role)) {
      ...
      console.log(
        "Step 5: Skipping AMS profile creation (role is not ADMIN/NUTRITIONIST)",
      );
    }
...
export async function login(req, res) {
  try {
    ...
    console.log(
      `Verification code generated (expires in ${CODE_EXPIRY_MINUTES} minutes)`,
    );
...
export async function verifyCode(req, res) {
  try {
    ...
    const verification = await services.getVerificationCodeByUserId(
      user.id,
      "LOGIN_2FA",
    );
    ...
    console.log(
      `Attempts: ${verification.attempts}/${MAX_VERIFICATION_ATTEMPTS}`,
    );
