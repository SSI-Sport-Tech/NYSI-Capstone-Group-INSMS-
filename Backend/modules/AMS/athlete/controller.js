...
export async function getAthleteProfile(req, res) {
    try {
        ...
        res.status(500).json({ error: 'Failed to fetch athlete profile', message: error.message });
    }
}
...
export async function createCompleteAthlete(req, res) {
    try {
        ...
        // Step 5: Look up nutritionist from logged-in user for auto-assignment
        ...
        const nutritionistIds = [nutritionistId];
        ...
        console.log(`Step 7: Athlete ${result.athlete.id} created successfully with all relations`);
...
export async function adminCreateCompleteAthlete(req, res) {
    try {
        ...
        // Step 5: Validate nutritionist_id exists
        ...
        const result = await services.createCompleteAthlete(
            athleteData, registryData, medicalData,
            validated.coach_ids, [validated.nutritionist_id]
        );

        console.log(`Step 7: Athlete ${result.athlete.id} created successfully with all relations (admin)`);
...
export async function updateAthleteProfile(req, res) {
    try {
        ...
        const exists = await pool.query('SELECT id FROM AMS.Athlete WHERE id = $1', [id]);
        ...
        if (validated.coach_ids && validated.coach_ids.length > 0) {
            ...
            if (coachCheck.rows.length !== validated.coach_ids.length) {
                const foundIds = coachCheck.rows.map(r => r.id);
                const invalidIds = validated.coach_ids.filter(cid => !foundIds.includes(cid));
                return res.status(400).json({
                    error: 'Invalid coach_ids',
                    details: [{ field: 'coach_ids', message: `Coach(es) not found: ${invalidIds.join(', ')}` }],
                });
        ...
        const result = await services.updateAthleteProfile(id, validated, { updateNutritionists: false });
        ...
        res.status(500).json({ error: 'Failed to update athlete profile', message: error.message });
    }
}
    ...
    try {
        ...
        if (validated.nutritionist_ids && validated.nutritionist_ids.length > 0) {
            ...
            if (nutritionistCheck.rows.length !== validated.nutritionist_ids.length) {
                ...
                const invalidIds = validated.nutritionist_ids.filter(nid => !foundIds.includes(nid));
                return res.status(400).json({
        ...
        const result = await services.updateAthleteProfile(id, validated, { updateNutritionists: true });
